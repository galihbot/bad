 ...
 */
 function ytv(url) {
    return new Promise(async (resolve, reject) => {
		...
		let b = await fetchJson(`https://api.lolhuman.xyz/api/ytvideo2?apikey=${global.lolkey}&url=${url}`)
...
function yta(url) {
    return new Promise(async (resolve, reject) => {
        if (ytIdRegex.test(url)) {
		let b = await fetchJson(`https://api.lolhuman.xyz/api/ytaudio2?apikey=${global.lolkey}&url=${url}`)
			if (b.status !== 200) return 'Web Sedang Gangguan'
		const {title, thumbnail } = b.result
            resolve({
                                dl_link: b.result.link,
